<template>
  <swiper
    :slides-per-view="1"
    class="bg-swiper"
    @slideChange="Root.onSlideChange"
    ref="swiperRef"
    @swiper="onSwiper"
    loop
  >
    <swiper-slide
      v-for="(item, index) in Root.list"
      :key="index"
      class="bg-slide"
    >
      <div
        class="bg-image"
        :style="{
          background: `url(${getImgUrl(`kami/kami_${index + 1}.webp`)}) center / cover no-repeat`,
        }"
      ></div>

      <div class="message-container">
        <div
          v-for="(line, lineIdx) in item.message"
          :key="lineIdx"
          class="message-line"
        >
          <span
            ...
            :key="charIdx"
            class="char"
            :style="{ '--delay': `${lineIdx * 2.0 + charIdx * 0.24}s` }"
          >
            {{ char === ' ' ? '\u00A0' : char }}
          </span>
        </div>
